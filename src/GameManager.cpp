...
GameManager::GameManager()
    : window(nullptr), renderer(nullptr), isRunning(false), mapLoader(nullptr), player(nullptr)
{
...
void GameManager::setupEventHandlers()
{
    ...
    eventBus.subscribe(GameEventType::PLAYER_MOVED,
...
bool GameManager::init(SDL_Window *existingWindow, SDL_Renderer *existingRenderer)
{
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0)
        return false;
...
bool GameManager::loadTilesets()
{
    for (int i = 0; i < mapLoader->getNumTilesets(); ++i)
    {
        ...
        if (!texture)
        {
            std::cerr << "Failed to create texture from tileset: " << imagePath << std::endl;
            return false;
        }
...
void GameManager::renderLayer(int layerIndex)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            SDL_Rect dest = {x * tileset->GetTileWidth(), y * tileset->GetTileHeight(),
