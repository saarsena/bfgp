...
Player::Player(int tileX, int tileY, SDL_Texture *texture, entt::registry &registry, int tileSize)
...
{
    SDL_Rect dstRect = {position.x * tileSize, position.y * tileSize, tileSize, tileSize};
...
bool Player::move(int dx, int dy, EventBus &bus)
{
    ...
    if (!CollisionSystem::CheckCollision(registry, newPos, entity))
    {
        ...
        registry.patch<Components::PositionComponent>(entity, [&](auto &pos) { pos = newPos; });
